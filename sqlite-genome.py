    ...
    def dbprepare(gfffile, prepare):
        ...
        for i in range(len(genomeloc)):
            dbprepare[genomeloc[i]] = [{genomeloc[i]: accession[i]}, {genomeloc[i]: typeaccession[i]},
                                       {genomeloc[i]: start[i]}, {
                                           genomeloc[i]: end[i]},
                                       {genomeloc[i]: length[i]}, {
                                           genomeloc[i]: posstrand[i]},
         ...
         """)
            ...
            INSERT INTO typeaccession VALUES
                     tuple(([typeaccession[i]
                           for i in range(len(typeaccession))]))
         """)
                connection.commit()
                connection.execute("""
            ...
            for i in range(len(exonseqkeys)):
                for j in range(len(fasta_seq)):
                    if exonseqkeys[i] == fasta_names[i]:
                        exonseqprepare[exonseqkeys[i]
                                       ] == fasta_seq[i][exonseqvalues[i][0]:exonseqvalues[i][1]]
        connection = con.cursor()
            ...
            for i in range(len(intronseqkeys)):
                for j in range(len(fasta_seq)):
                    if intronseqkeys[i] == fasta_names[i]:
                        intronseqprepare[intronseqkeys[i]
                                         ] == fasta_seq[i][intronseqvalues[i][0]:intronseqvalues[i][1]]
        connection = con.cursor()
        ...
        prepareontologycomb5 = []
            ...
            for line in gowrite.readlines():
                if len(line) <= 3:
                    ...
                    prepareontologycomb1.append(
                        {line.strip().split("\t")[0]: line.strip().split("\t")[1]})
                    prepareontologycomb2.append(
                        {line.strip().split("\t")[0]: line.strip().split("\t")[2]})
                    prepareontologycomb3.append(
                        {line.strip().split("\t")[0]: line.strip().split("\t")[3]})
                    prepareontologycomb4.append(
                        {line.strip().split("\t")[1]: line.strip().split("\t")[2]})
                    prepareontologycomb5.append(
